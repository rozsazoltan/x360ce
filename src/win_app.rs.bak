            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ...
                        ui.add_space(12.0);
                        self.render_device_bar(ui);
                        ui.add_space(12.0);

                        ui.columns(2, |columns| {
                            let (left, right) = columns.split_at_mut(1);
                            left[0].set_width(left[0].available_width());
                            self.render_controller(&mut left[0]);
                            self.render_mapping_editor(&mut right[0]);
                        });

                        ui.add_space(12.0);
                        self.render_raw_inputs(ui);
                        ui.add_space(12.0);
...
impl X360ceApp {
    fn render_header(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            draw_header_logo(ui);
            ui.vertical(|ui| {
                ui.label(RichText::new("x360ce").size(28.0).strong().color(TEXT));
                ui.label(
                    RichText::new("Physical controller → virtual Xbox 360 controller")
                        .size(13.0)
                        .color(MUTED),
                );
            });
    ...
    fn render_status(&mut self, ui: &mut egui::Ui) {
        ...
        surface(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    let text = if self.state.emulation_enabled {
                        "Disable"
                    } else {
                        "Enable"
                    };
                    ...
                    if !self.runtime.driver_installed && ui.button("Install ViGEmBus").clicked() {
                        match driver::launch_installer_elevated() {
                            Ok(_) => {
                                self.status = "Driver installer opened. Restart x360ce after install.".to_owned()
                            }
    ...
    fn render_mapping_editor(&mut self, ui: &mut egui::Ui) {
        surface(ui, |ui| {
            ...
            ui.label(
                RichText::new(format!("{} · {}", profile.name, profile.device_guid))
                    .size(10.0)
                    .color(MUTED),
            );
            let entry = profile.entry(control).cloned().unwrap_or_else(|| {
                crate::model::MappingEntry::new(control, InputBinding::None)
            });
            ui.label(RichText::new(entry.binding.label()).color(MUTED));
            ui.add_space(10.0);

            let learning = self.learning_control == Some(control);
            ui.horizontal(|ui| {
                if ui
                    .add_enabled(!learning, egui::Button::new("Learn input"))
                    .clicked()
                {
            ...
            if control.is_axis() || control.is_trigger() {
                ...
                let mut changed = false;
                {
                    ...
                    if control.is_trigger() {
                        changed |= ui
                            .checkbox(
                                &mut mapping.centered_axis,
                                "Centered/split source axis",
                            )
                            .on_hover_text(
                                "Enable when two triggers share one axis centered at zero.",
                            )
                            .changed();
                    }
                    changed |= ui
                ...
                let value = self
                    ...
                    .unwrap_or_default();
                ui.add_space(8.0);
                ui.label(format!("Live value: {value:.2}"));
                ...
                .show(ui, |ui| {
                        ...
                        .chain(OutputControl::ANALOGS)
                    {
                        ...
                        if ui
                            .selectable_label(
                                self.selected_control == control,
                                format!("{:<18}  {}", control.label(), binding),
                            )
                            .clicked()
                        {
                            self.selected_control = control;
    ...
    fn render_settings(&mut self, ui: &mut egui::Ui) {
        surface(ui, |ui| {
            ui.horizontal(|ui| {
                ui.vertical(|ui| {
                    ui.label(RichText::new("Tray and startup").strong());
                    let mut changed = false;
                    changed |= ui
                        .checkbox(
                            &mut self.state.auto_tray_enabled,
                            "Move UI to tray after inactivity",
                        )
                        .changed();
                    changed |= ui
                        .checkbox(&mut self.state.start_in_tray, "Start directly in tray")
                        .changed();
                    changed |= ui
                        .add(
                            egui::Slider::new(
                    ...
                    let startup_label = if self.state.startup_enabled {
                        "Disable Windows startup"
                    } else {
                        "Enable Windows startup"
                    };
                    if ui
                        .add_enabled(!config::is_dev_build(), egui::Button::new(startup_label))
                        .clicked()
                    {
                        self.toggle_startup();
                    }
                ...
                ui.vertical(|ui| {
                    ...
                    if ui
                        .checkbox(
                            &mut self.state.include_prereleases,
                            "Include prereleases",
                        )
                        .changed()
                    {
                ...
                ui.vertical(|ui| {
                    ui.label(RichText::new("Portable data").strong());
                    ...
                    ui.label("All profiles stay beside executable in .x360ce-data/config.json.");
                });
            });
        });
    }
