...
use windows_sys::Win32::{
    System::Registry::{RegCloseKey, RegOpenKeyExW, HKEY, HKEY_LOCAL_MACHINE, KEY_QUERY_VALUE},
...
pub fn launch_installer_elevated() -> Result<PathBuf> {
    ...
    if result <= 32 {
        anyhow::bail!(
            "failed to launch ViGEmBus installer with elevation (ShellExecuteW code {result})"
        );
    }
...
fn extract_installer() -> Result<PathBuf> {
    ...
    fs::write(&path, INSTALLER_BYTES).with_context(|| {
        format!(
            "failed to extract embedded driver installer: {}",
            path.display()
        )
    })?;
    Ok(path)
